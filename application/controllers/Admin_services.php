...
class Admin_services extends MY_Controller
{
        ...
        {
                parent::__construct();
                $this->load->model('manage_su/Model_su_services');
        }

        private function auto_id()
        {
                return $this->Model_su_services->announce_id_auto_model();
        }
        ...
        public function Add_Data_ctl()
        {
                ...
                $this->Model_su_services->announce_add_model($data);
                echo json_encode(true);
        }

        public function del_data_ctl(){
                $data = $this->input->post('del_anc');
                $this->Model_su_services->announce_del_model($data);
                echo json_encode(true);
        }

        public function announce_showdata_ctl()
        {
                $result = $this->Model_su_services->announce_showdata_model();
        ...
        public function edit_data_ctl()
        {
                ...
                $this->Model_su_services->announce_edit_model($dataid, $data);
                echo json_encode(true);
        }
}
