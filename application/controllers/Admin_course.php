...
class Admin_course extends MY_Controller
{
        ...
        {
                parent::__construct();
                $this->load->model('manage_su/Model_su_course');
        }

        private function auto_id()
        {
                return $this->Model_su_course->announce_id_auto_model();
        }
        ...
        public function Add_Data_ctl()
        {
                ...
                $this->Model_su_course->announce_add_model($data);
                echo json_encode(true);
        }

        public function del_data_ctl(){
                $data = $this->input->post('del_anc');
                $this->Model_su_course->announce_del_model($data);
                echo json_encode(true);
        }

        public function announce_showdata_ctl()
        {
                $result = $this->Model_su_course->announce_showdata_model();
        ...
        public function edit_data_ctl()
        {
                ...
                $this->Model_su_course->announce_edit_model($dataid, $data);
                echo json_encode(true);
        }
}
