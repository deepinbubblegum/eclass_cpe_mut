...
class Model_su_announce extends CI_Model
{
        public function announce_id_auto_model()
        {
                $query = $this->db->query('SELECT CONCAT("ANC-",LPAD(SUBSTRING(IFNULL(MAX(anc_id), "0"), 5,6)+1, 6,"0")) as anc_id FROM announce_su;');
                if ($query->num_rows() > 0) {
                        $row = $query->row_array();
                        return $row['anc_id'];
                }
        ...
        {
                $this->db->insert('announce_su', $args);
        }
