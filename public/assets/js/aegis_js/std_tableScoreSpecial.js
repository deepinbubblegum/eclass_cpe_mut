$(document).ready(function() {
    ...
    function showTableHead() { //tableScoreZone
        takeThisUrl = '/' + url[3] + '/Std_table_score/showTableHeader';
        $.ajax({
            type: "POST",
            url: takeThisUrl,
    ...
    function genOverHead() { //tableScoreZone
        takeThisUrl = '/' + url[3] + '/Std_table_score/showOverHeader';
        $.ajax({
            type: "POST",
            url: takeThisUrl,
            data: '&semester=' + semester + '&subject_id=' + subject_id + '&parent_id=' + point_id,
    ...
    function showTableBody() { //tableScoreZone
        takeThisUrl = '/' + url[3] + '/Std_table_score/showTableBodySpecial';
        $.ajax({
            type: "POST",
            url: takeThisUrl,
                ...
                $.each(getFieldPoint, function(i, p) {
                    $("#charts-" + i).click(function(e) {
                        ...
                        for (j = 0; j < bodyData.length; j++) {
                            getData[j] = $('#point-' + bodyData[j].std_code_id + '-' + getFieldPoint[i].setpoint_setpoint_id).text();
                        }
            ...
            for (i = loopStart; i < loopStop; i++) { //FIELD 
                for (j = 0; j < bodyData.length; j++) { //STD  
                    ...
                    if (pointData[i].setpoint_option == '2') {
                        ...
                        takeFormula(bodyData[j].std_code_id, pointData[i].setpoint_setpoint_id, setFormula, getSumMaxFormula);
        ...
        for (i = pointData.length - formulaMax + F34R; i < pointData.length + (F34R - 1); i++) { //FIELD 
            for (j = 0; j < bodyData.length; j++) { //STD   
                if (pointData[i].setpoint_option == '2') {
                    ...
                    setFormula = htmlEncodeF34R(setFormula);
                    takeFormula(bodyData[j].std_code_id, pointData[i].setpoint_setpoint_id, setFormula);
                }
