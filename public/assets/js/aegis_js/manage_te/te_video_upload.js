$(document).ready(function() {
    ...
    $('#create_menu_video').click(function(e) {
        e.preventDefault();
        MenuName = $('#CMenuVideo').val();
        Discrip = $('#discription_menu').val();
        if (MenuName == '') {
            $('#CMenuVideo').addClass('is-invalid');
            return false;
        } else {
            $('#CMenuVideo').removeClass('is-invalid');
        }

        $.ajax({
            type: "POST",
            url: "/" + url[3] + "/te_video/addmenu_video",
            data: "&menu_video=" + MenuName + "&discrip=" + Discrip + "&subject_id=" + subject_id + "&semester=" + semester,
            dataType: "json",
            success: function(response) {
                showMenuUpload();
                $('#addMenuVideo').modal('hide');
                $('#CMenuVideo').val('');
    ...
    function showMenuUpload() {
        $.ajax({
            type: "POST",
            url: "/" + url[3] + "/te_video/show_menu_video_up",
    ...
    function uploadBtn(pos) {
        ...
        $.ajax({
            type: "POST",
            url: '/' + url[3] + '/Te_video/UploadFile/' + subject_id + '-' + semester + '-' + getMenu[pos].menuDowId,
            data: form_data,
            contentType: false,
            cache: false,
            processData: false,
